...
static void proc_K0(hls::stream<wide_t>& from_input, 
    ...
    for (int i = 0; i < N / 32; i++) {
        ...
        for (int j = 0; j < 32; j++) {
            ...
            #pragma HLS BIND_OP variable=res op=mul impl=dsp latency=4
...
static void proc_K1(hls::stream<wide_short_t>& from_k0, hls::stream<wide_short_t>& to_k3) {
    ...
    for (int i = 0; i < N / 32; i++) {
        ...
        for (int j = 0; j < 32; j++) {
            ...
            #pragma HLS BIND_OP variable=m0 op=mul impl=dsp latency=4
            short_data_t m1 = x1 * w1;
            #pragma HLS BIND_OP variable=m1 op=mul impl=dsp latency=4
            short_data_t m2 = x2 * w2;
            #pragma HLS BIND_OP variable=m2 op=mul impl=dsp latency=4
    ...
    for (int b = 0; b < N / BLOCK; b++) {
        ...
        for (int i = 0; i < BLOCK / 32; i++) {
            ...
            for (int j = 0; j < 32; j++) {
                ...
                // STAGE 1 FENCE
                #pragma HLS BIND_OP variable=a op=add impl=dsp latency=4
                abs_v[j] = a;
            }
            ...
            for(int j=0; j<8; j++) {
                ...
                #pragma HLS BIND_OP variable=sum_l2 op=add impl=dsp latency=4
                s2[j] = sum_l2;
            }
            ...
            #pragma HLS BIND_OP variable=block_sum op=add impl=dsp latency=4
            total_sum += block_sum;
        }
...
static void proc_K4(hls::stream<wide_short_t>& from_k3, hls::stream<wide_t>& to_output) {
    ...
    for (int i = 0; i < N / 32; i++) {
        ...
        for (int j = 0; j < 32; j++) {
            ...
            #pragma HLS BIND_OP variable=z op=mul impl=dsp latency=4
...
#pragma HLS interface m_axi port=in offset=slave bundle=in \
    max_read_burst_length=32 num_read_outstanding=32 latency=64 \
    max_widen_bitwidth=1024
#pragma HLS interface m_axi port=out offset=slave bundle=out \
    max_write_burst_length=32 num_write_outstanding=32 latency=64 \
    max_widen_bitwidth=1024
#pragma HLS interface s_axilite port=return
    ...
    #pragma HLS stream variable=s0_k1 depth=32
    #pragma HLS stream variable=s0_k2 depth=32
    #pragma HLS stream variable=s3_k4 depth=32
    #pragma HLS stream variable=raw_in depth=32
    #pragma HLS stream variable=raw_out depth=32
